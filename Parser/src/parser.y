...
parametersList		: PARAMETER { $$ = creaParametri(NULL,$1); }
			| PARAMETER COMMA parametersList { $$ = creaParametri($3,$1); }
			;

ruleSet 		: /* Empty */ {$$ = NULL;}
			| wellFormedFormula THEN fuzzyAssignment END_RULE ruleSet {$$ = aggiungiAllaLista($5, $1, $3); rulebaseRoot = $$;}
			;

wellFormedFormula	: fuzzyAssignment {$$ = $1;}
			| OPEN_B wellFormedFormula CLOSE_B {$$ = $2;}
			| OP_NOT wellFormedFormula {$$ = creaNodo(NOT_N, NULL, $2, "");}
			| wellFormedFormula OP_OR wellFormedFormula {$$ = creaNodo(OR_N, $1, $3, "");}
