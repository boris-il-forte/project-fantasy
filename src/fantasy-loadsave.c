...
t_lista_t * inserisci_truppe_in_coda(t_lista_t *testa, FILE *fp, char from) //# stavolta faccio notare che è meglio in questa funzione fare solo l'inserimento e non cercare la coda! inutile cercare la coda in quanto nessun'altra parte del codice usa la suddetta funzione.
{
	...
	ckfread(&nuova->truppa->combattuto, sizeof(nuova->truppa->combattuto), 1,
			fp);
...
int carica(char *nomefile)
{
	...
	{ // *giocatore
		...
		{ // **struttura
			...
			{ // *struttura(scorri)
				giocatore[i]->struttura[j] = inserisci_strutture_in_coda(
						giocatore[i]->struttura[j], fp);
			} // END *struttura
		}
		ckfread(&num_truppe, sizeof(num_truppe), 1, fp);
		giocatore[i]->truppe = NULL;
		for (j = 0; j < num_truppe; j++)
		{ // *truppe(scorri)
			giocatore[i]->truppe = inserisci_truppe_in_coda(
					giocatore[i]->truppe, fp, 0);
		} // END *truppe
	...
	{ // *giocatore(scorri)
		...
		{ // **struttura
			...
			{ // *struttura(scorri)
				...
				Sptrbis = Sptr->in;
				for (l = 0; Sptrbis != NULL; l++)
				{ // *in(conta lista)
					Sptrbis = Sptrbis->next;
				}
				num_truppestruttura = l;
				ckfwrite(&num_truppestruttura, sizeof(num_truppestruttura), 1,
						fp);
				Sptrbis = Sptr->in;
				for (l = 0; l < num_truppestruttura; l++)
				{ // *in(scorri)
					ckfwrite(&Sptrbis->truppa->tipo,
							sizeof(Sptrbis->truppa->tipo), 1, fp);
					ckfwrite(&Sptrbis->truppa->giocatore,
							sizeof(Sptrbis->truppa->giocatore), 1, fp);
					ckfwrite(&Sptrbis->truppa->numero,
							sizeof(Sptrbis->truppa->numero), 1, fp);
					ckfwrite(&Sptrbis->truppa->morale,
							sizeof(Sptrbis->truppa->morale), 1, fp);
					ckfwrite(&Sptrbis->truppa->stanca,
							sizeof(Sptrbis->truppa->stanca), 1, fp);
					ckfwrite(&Sptrbis->truppa->combattuto,
							sizeof(Sptrbis->truppa->combattuto), 1, fp);
		...
		{ // *truppe(scorri)
			...
			ckfwrite(&Tptr->truppa->giocatore, sizeof(Tptr->truppa->giocatore),
					1, fp);
			ckfwrite(&Tptr->truppa->numero, sizeof(Tptr->truppa->numero), 1,
					fp);
			ckfwrite(&Tptr->truppa->morale, sizeof(Tptr->truppa->morale), 1,
					fp);
			ckfwrite(&Tptr->truppa->stanca, sizeof(Tptr->truppa->stanca), 1,
					fp);
			ckfwrite(&Tptr->truppa->combattuto,
					sizeof(Tptr->truppa->combattuto), 1, fp);
		...
		ckfwrite(&giocatore[i]->smeraldi, sizeof(giocatore[i]->smeraldi), 1,
				fp);
	} // END *giocatore
